...
class FunSetSuite extends FunSuite {
  ...
  test("filter filters") {
    new TestSets {
      ...
      val filtered = filter(all, (x:Int) => x > 1)
  ...
  test("forall"){
    new TestSets {
      val all = union(s1, union(s2, s3))
      assert(forall(all, _ > 0), "Forall true")
      assert(!forall(all, _ < 3), "Forall false")
    }
  }
  ...
  test("exists"){
    new TestSets {
      val all = union(s1, union(s2, s3))
      assert(exists(all, _ % 2 == 0), "Exists true")
      assert(!exists(all, _ < 0), "Exists false")
    }
  }
  ...
  test("map") {
    new TestSets {
      val all = union(s1, union(s2, s3))
      val mapped = map(all, _+1)
      assert(contains(mapped, 4), "Map 1")
      assert(!contains(mapped, 1), "Map 2")
    }
  }
